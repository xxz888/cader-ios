...
@property(nonatomic, strong) STModalWindow *window;

@end

@implementation STModal
- (void)setCoverFrame:(CGRect)coverFrame {
    _coverFrame = coverFrame;
    [self window].window.frame = coverFrame;
}
+ (instancetype)modal{
    return [self new];
}
...
//    return [STModalWindow sharedModalWindow];
    if (!_window) {
        _window = [[STModalWindow alloc] init];
    }
    return _window;
}
...
- (void)tapped{
    ...
    if (self.didTouchOutsideControlBlock) {
        self.didTouchOutsideControlBlock();
    }
}
...
- (void)transitionFromModal:(STModal *)fromModal
    ...
    if (fromA == toA && colosIsEqual(fromColor, toColor)){
        ...
        [UIView animateWithDuration:animated?duration:0
                         animations:^{
            self->_dimBackgroundView.alpha = toA;
            self->_dimBackgroundView.backgroundColor = toColor;
                         }
